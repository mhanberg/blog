defmodule Blog.BookshelfPage do
  ...
  def template(assigns) do
    ...
    temple do
      ...
      h2 class: "text-center underline mt-16 text-lg", do: "Currently Reading"

      div class: "mt-8 font-mono" do
        c &books/1, books: @currently_reading

        for {year, books} <- @years do
          h2 class: "text-center mt-16 text-lg", do: "#{year} (#{length(books)} books)"
  ...
  defp book(assigns) do
    temple do
      a href: @book["link"] || "#",
        target: "_blank",
        class:
          "book [&:not(:only-child)]:last:rotate-[-4deg] [&:not(:only-child)]:last:-translate-x-2 flex items-center text-sm py-4 bg-green-700 h-[250px] w-[50px] [writing-mode:vertical-rl] rounded" do
