defmodule Blog.BookshelfPage do
  ...
  def template(assigns) do
    ...
    combos =
      for color <- colors, height <- heights do
        {color, height}
      end
    ...
    temple do
      c &prose/1 do
        ...
        I pull this data from my [Hardcover](https://hardcover.app/@mitchhanberg) account once a day.
        """)

        h2 class: "mt-16 text-center underline", do: "Currently Reading"

        div class: "font-mono mt-8" do
          c &books/1, books: @currently_reading, combos: @combos

          for {year, books} <- @years do
            h2 class: "mt-16 text-center", do: "#{year} (#{length(books)} books)"

            c &books/1, books: books, combos: @combos
          end
        end
      end
    end
  ...
  defp books(assigns) do
    temple do
      div class:
        ...
        for book <- @books do
          c &book/1, book: book, combos: @combos
        end
      end
    end
  end
  ...
  defp book(assigns) do
    idx = :erlang.phash2(assigns.book["title"], length(assigns.combos))

    {color, height} = Enum.at(assigns.combos, idx)
    ...
    temple do
      a href: @book["link"] || "#",
        title: @book["title"],
        target: "_blank",
        style: "width: clamp(calc(0.1 * #{@book["pages"]}px), 40px, 250px)",
        class:
          "#{@color} #{@height} book [writing-mode:vertical-rl] inset-shadow-x flex items-center justify-between rounded border-t border-white text-sm no-underline decoration-black last:[&:not(:only-child)]:rotate-[-4deg] last:[&:not(:only-child)]:translate-x-2" do
        div class:
