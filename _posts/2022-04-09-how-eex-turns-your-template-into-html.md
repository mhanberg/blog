...
If the encountered expression is multi-line, it will call the `c:handle_begin/3` callback. This creates a new "nesting", and will will return a brand new state and should collect the output for the "nested" tokens.
...
Here is what the compiled output for the above example.
...
We can observe some similarities: it has the same `@` assigns functionality, it groups dynamic pieces into variables to coordinate later into the final result.
